...
pos = @(t) [mod(floor(t), 2) * 0; -mod(floor(t/2), 2) * pi/6];
...
plot(T, angles, '-')
plot(T, POS', '--')
legend('alpha', 'beta', 'alpha des', 'beta des')
title('Tail position vs desired')
...
% tau = 0;
tau = [-(alpha - pos(1)) * 10 - dalph * 1;-(beta - pos(2)) * 100 - dbeta * 5];
